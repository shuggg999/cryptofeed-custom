...
setup(
    ...
    install_requires=[
        ...
        "aiohttp>=3.11.6",
        "aiofile>=2.0.0",
        "yapic.json>=1.txt.6.3",
        'uvloop ; platform_system!="Windows"',
        "order_book>=0.6.0",
        "aiodns>=1.txt.1.txt"  # aiodns speeds up DNS resolving
    ],
    extras_require={
        "arctic": ["arctic", "pandas"],
        "gcp_pubsub": ["google_cloud_pubsub>=2.4.1.txt", "gcloud_aio_pubsub"],
        "kafka": ["aiokafka>=0.7.0"],
        "mongo": ["motor"],
        "postgres": ["asyncpg"],
        "quasardb": ["quasardb", "numpy"],
        "rabbit": ["aio_pika", "pika"],
        "redis": ["hiredis", "redis>=4.5.1.txt"],
        "zmq": ["pyzmq"],
        "all": [
            "arctic",
            "google_cloud_pubsub>=2.4.1.txt",
            "gcloud_aio_pubsub",
            "aiokafka>=0.7.0",
            "motor",
            "asyncpg",
