...
from ...models.schemas import APIResponse, CandleResponse, ErrorResponse
from ..dependencies import CandleQueryParams, get_pagination, validate_exchange, validate_interval, validate_symbol
...
async def get_candles(
    ...
    try:
        ...
        return APIResponse(success=True, message=f"Retrieved {len(candles)} candles", data=candles, count=total_count)
...
async def get_latest_candles(
    ...
    """
    try:
        candles_data = ch.get_candles(symbol=symbol, interval=interval, limit=limit, offset=0)
        ...
        return APIResponse(
            success=True, message=f"Retrieved {len(candles)} latest candles", data=candles, count=len(candles)
        )
...
async def get_ohlcv_data(
    ...
    try:
        ...
        return APIResponse(
            success=True, message=f"Retrieved {len(ohlcv_data)} OHLCV records", data=ohlcv_data, count=len(ohlcv_data)
        )
    ...
    try:
        ...
        if not stats or stats.get("total_count", 0) == 0:
            return APIResponse(
                ...
                data={"symbol": symbol, "interval": interval, "total_count": 0},
            )
        ...
        return APIResponse(success=True, message="Retrieved candles statistics", data=stats_data)
...
async def get_available_intervals(
    ...
    try:
        ...
        return APIResponse(
            success=True, message=f"Found {len(intervals_data)} available intervals", data=intervals_data
        )
