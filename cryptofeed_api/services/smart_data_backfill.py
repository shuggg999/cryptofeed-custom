...
class SmartDataBackfillService:
    ...
    def __init__(self, config_path: str = "/app/config/main.yaml"):
        ...
        self.priority_weights = {
            ...
            "interval": {"1m": 10, "5m": 8, "15m": 6, "30m": 5, "1h": 4, "4h": 3, "1d": 1},
        }
        ...
        try:
            ...
            for symbol in symbols:
                for interval in intervals:
                    status = await self._quick_check_single_source(client, symbol, interval)
    ...
    async def _quick_check_single_source(self, client, symbol: str, interval: str) -> str:
        """快速检查单个数据源"""
        # 获取保留天数
    ...
    async def _detect_precise_gaps(self, scenario: str = "normal") -> List[PreciseGap]:
        ...
        try:
            ...
            for symbol in symbols:
                for interval in intervals:
                    gaps = await self._find_gaps_for_source(client, symbol, interval, scenario)
    ...
    async def _find_gaps_for_source(
        ...
        result = client.query(sql, {"symbol": symbol, "interval": interval, "start": expected_start, "end": now})

        if not result.result_rows:
            # 完全没有数据
            gap = self._create_gap(symbol, interval, expected_start, now, interval_minutes)
            gaps.append(gap)
            return gaps
        ...
        if first_time > expected_start:
            gap = self._create_gap(symbol, interval, expected_start, first_time, interval_minutes)
            gaps.append(gap)
        ...
        for i in range(len(existing_times) - 1):
            ...
            if next_time > expected_next + timedelta(minutes=interval_minutes):
                gap = self._create_gap(symbol, interval, expected_next, next_time, interval_minutes)
                gaps.append(gap)
        ...
        if last_time < expected_latest:
            gap = self._create_gap(
                symbol, interval, last_time + timedelta(minutes=interval_minutes), now, interval_minutes
            )
            gaps.append(gap)
    ...
    def _create_gap(
        self, symbol: str, interval: str, start: datetime, end: datetime, interval_minutes: int
    ) -> PreciseGap:
        """创建缺口对象"""
    ...
    def _calculate_priority(self, gap_type: GapType, interval: str, duration_hours: float) -> int:
        """计算回填优先级"""
        # 时间权重
        ...
        # 综合优先级（1-10分）
        priority = min(10, max(1, time_weight * 0.6 + interval_weight * 0.3 + duration_weight * 0.1))

        return int(priority)

    def _classify_gaps_by_priority(self, gaps: List[PreciseGap]) -> Dict[str, List[PreciseGap]]:
        """按优先级分类缺口"""
        classified = {"urgent": [], "recent": [], "historical": []}
    ...
    async def _execute_prioritized_backfill(self, classified_gaps: Dict[str, List[PreciseGap]]) -> Dict[str, any]:
        """执行分级回填 - 紧急优先，历史延后"""
        results = {
            "urgent_completed": 0,
            "recent_completed": 0,
    ...
    async def _fill_gaps_batch(self, gaps: List[PreciseGap]) -> Dict[str, any]:
        """批量填充缺口 - 复用现有的API调用逻辑"""
        results = {"completed": 0, "records_filled": 0, "errors": []}
    ...
    async def _fill_single_gap(self, gap: PreciseGap) -> int:
        ...
        while current_start < end_ms:
            ...
            try:
                ...
                if klines_data:
                    # 插入数据（复用原有的数据处理逻辑）
                    inserted = await self._insert_klines_data(klines_data, gap.symbol, gap.interval)
    ...
    async def _insert_klines_data(self, klines_data: List, symbol: str, interval: str) -> int:
        """插入K线数据 - 复用原有的数据插入逻辑"""
        client = clickhouse_connect.get_client(
            host=self.clickhouse_cfg["host"],
            port=self.clickhouse_cfg["port"],
        ...
        try:
            ...
            if insert_data:
                client.insert(
                    "candles", insert_data, column_names=list(normalized_data.keys()) if normalized_data else []
                )
    ...
    async def _handle_scenario_preprocessing(self, scenario: str) -> Dict[str, any]:
        ...
        if scenario == "startup":
            ...
            scenario_info["downtime_duration"] = (
                (datetime.now() - last_check).total_seconds() / 3600 if last_check else None
            )
            logger.info(
                f"🔄 重启场景检测: 上次检查时间 {last_check}, 停机时长 {scenario_info['downtime_duration']:.1f}小时"
            )
    ...
    async def _detect_scenario_specific_gaps(self, scenario: str, scenario_info: Dict[str, any]) -> List[PreciseGap]:
        """场景特定的缺口检测"""
        additional_gaps = []
    ...
    async def _check_network_disruption_status(self) -> Dict[str, any]:
        ...
        try:
            ...
            return {"disrupted_count": len(disrupted_sources), "disrupted_sources": disrupted_sources}
        ...
        try:
            ...
            for symbol in symbols:
                for interval in intervals:
                    ...
                    result = client.query(sql, {"symbol": symbol, "interval": interval, "check_time": check_time})
    ...
    async def _detect_startup_gaps(self, scenario_info: Dict[str, any]) -> List[PreciseGap]:
        ...
        for symbol in symbols:
            for interval in intervals:
                # 检查停机期间是否有缺口
                gap = self._create_gap(symbol, interval, last_check, now, self._get_interval_minutes(interval))
    ...
    async def _detect_network_gaps(self, scenario_info: Dict[str, any]) -> List[PreciseGap]:
        ...
        for source in network_status.get("disrupted_sources", []):
            ...
            if last_ws_time:
                ...
                gap = self._create_gap(
                    symbol, interval, last_ws_time, datetime.now(), self._get_interval_minutes(interval)
                )
    ...
    async def _detect_manual_deletion_gaps(self, scenario_info: Dict[str, any]) -> List[PreciseGap]:
        ...
        for anomaly in anomalies:
            ...
            # 为数据异常的时间范围创建缺口
            gap = self._create_gap(
                symbol, interval, time_range["start"], time_range["end"], self._get_interval_minutes(interval)
            )
