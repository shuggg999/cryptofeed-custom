...
class BinanceRestClient:
    ...
    async def init_session(self):
        ...
        if not self.session:
            ...
            self.session = aiohttp.ClientSession(
                timeout=timeout, headers={"User-Agent": "Cryptofeed-API/1.0.0", "Content-Type": "application/json"}
            )
    ...
    async def get_klines(
        ...
        Returns:
            K线数据列表
        """
        params = {"symbol": symbol, "interval": interval, "limit": min(limit, 1500)}  # Binance限制
    ...
    async def get_trades(
        self, symbol: str, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None, limit: int = 1000
    ) -> List[Dict]:
        """
        获取交易数据
